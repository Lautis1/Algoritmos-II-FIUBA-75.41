...
void detectar_DOS_guardar_ips_en_abb(char* ip, void* solicitudes_ip, void* arbol_visitantes) {
    ...
    abb_guardar((abb_t*)arbol_visitantes, ip, NULL);
...
int comparar_ips(const char* ip_1, const char* ip_2){
	...
	int retorno;
	for(int i=0; i<CANT_POS_ARRAY_IP; i++){
...
void mostrar_mas_visitados(heap_t* recursos_mas_solicitados,  int n){
	...
	char* ruta = sitio_visitado->clave;
	printf("Sitios mas visitados: \n");
	printf("\t%s\n", ruta);
	mostrar_mas_visitados(recursos_mas_solicitados,n-1);
...
int procesar_entrada_stdin(char* linea_entrada, abb_t* arbol_visitantes, heap_t* recursos_mas_solicitados){
	...
	if(comparar_cadenas(input[0],AGREGAR_ARCHIVO) == 0){
		if(contar_cantidad_parametros(input) == CANT_PARAM_AGREGAR){
			procesar_log(input[0],recursos_mas_solicitados, "QUE SERIA SOLICITANTES EN LA FUNCION PROCESAR?");
		}
		else{
